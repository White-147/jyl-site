...
- **Performance budget**: `backdrop-filter` reduced to four fixed elements, infinite animations paused off-screen, particle count scaled to viewport, background layers merged from four to three; no first-paint content shift (CLS 0 on desktop)
...
├── docs/
│   ├── 联动维护点.md          # ★ Coupled-edit checklist (read before changing anything)
│   └── assets/screenshots/   # Site screenshot used by the README
├── public/
...
- **Preview icons**: every preview page and `404.html` declares a favicon plus a 180×180 apple-touch-icon (for large mobile history entries), generated by `scripts/gen-preview-icons.ps1` from each project's logo (SyLabAI uses the Shoayuan/Accela emblem). **Note**: the main site's own icons come from `scripts/gen_icons.py` — they are a separate set.
...
- **Three card tiers**: solid panels (project rows, skill and experience cards — opaque with a hairline), tinted panels (about and education cards — translucent, **no blur**), and frosted glass (sticky nav, bottom tab bar, side rail, lightbox only). The tier is decided by "does it need to show through?", not by importance.
...
- Scripted everything that can be scripted: `db:seed` / `db:export` for content, `fonts:subset` for type, `icons:gen` for icons, `contact:encode` for the obfuscation table, `watermark:resume` for the PDF, `previews:polish` for preview pages.
- Automated deployment: push-to-publish via GitHub Actions + GitHub Pages.
- Verified accessibility and stability: WCAG 2.2 AA contrast across seven viewports, zero layout shift on desktop, print interception.
