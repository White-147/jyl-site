...
export default function BackToTop() {
  ...
  return (
    <button
      ...
      className={`fixed bottom-20 right-5 z-50 flex h-12 w-12 items-center justify-center rounded-full border border-white/60 bg-white/75 text-brand-700 shadow-[0_6px_20px_rgba(157,83,0,0.28)] backdrop-blur-sm transition-all duration-300 [transition-timing-function:var(--ease-out-sharp)] hover:scale-110 active:scale-90 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-300 sm:h-14 sm:w-14 md:hidden dark:border-white/15 dark:bg-slate-900/70 dark:text-brand-200 dark:focus-visible:ring-brand-500/40 ${
      ...
      <svg viewBox="0 0 60 60" className="absolute inset-0 h-full w-full -rotate-90" aria-hidden="true">
        <defs>
          <linearGradient id="backtop-grad" x1="0%" y1="0%" x2="100%" y2="100%">
            {/* 与导航轨液柱同色（brand-700 → brand-400），保证两个进度指示器同源 */}
            <stop offset="0%" stopColor="#9d5300" />
            <stop offset="100%" stopColor="#f7bd6e" />
          </linearGradient>
        </defs>
