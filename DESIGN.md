...
### Icon + Title Cards
- ⚠️ **`<640px` 时图标与标题并排，图标盒 28px、图标 16px**（`max-sm:` 变体），`sm:` 以上回到竖排大图标
  （48px 盒 + 24px 图标）。用户实拍反馈：「手机端教育经历的第一个栏，图标独占一行，这很不美观」——
  竖排时 48px 图标独占一行、再夹 20px 的 flex gap，390 宽下卡片总高 180px，光图标那段就吃掉 68px。
- ⚠️ **图标要对齐到"标题那一行"，不是整块文字的垂直中心**：卡片是 `items-center` 时图标会掉到第二行的高度上，
  必须加 **`max-sm:self-start`**（实测加完图标与标题的 `y` 都是 252，正好同高）。
- 当前使用者：教育背景的学校卡（`Education.tsx`）。新增同类卡片时按这套值走，不要另起一套。
