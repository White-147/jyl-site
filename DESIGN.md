...
这是一份**档案**，不是一张海报。视觉系统的全部工作是把密集的技术履历组织成可信、可核对的条目：编号、发丝分隔线、对齐的元数据、克制的色彩。琥珀色承担全部意义载荷（可交互、可点击、当前项），其余一律退到纯中性石墨，其余一律退到带青调的暖灰里去。页面允许自己安静，因为它要证明的是"这个人做事有条理"，而不是"这个页面很漂亮"。
...
### Neutral
- **墨青** (`ink` `#0f2e36`): 浅色模式的标题色。带青调的近黑，绝不用纯黑。
- **暗场墨** (`ink-on-dark` `#dfeeec`): 深色模式正文与标题。刻意压到 15.6:1 而不是纯白，避免近黑底上的高亮晕影。
- **石墨画布** (`canvas-light` `#fafbfb`) / **深石墨画布** (`canvas-dark` `#121415`): 页面基色。深色画布比常见的 `#0a0a0a` 更青，和粒子液柱同族。
- **深色抬升面** (`surface-dark-raised` `#1b1e20`) / **深色卡片面** (`surface-dark-card` `#23282b`) / **深色发丝线** (`line-dark` `#17323c`): 深色模式的三级层级，全部在青玉族内，不用 Tailwind 默认的蓝灰 slate。
- **页脚石墨** (`footer-graphite` `#0d0f10`): 页脚专用。比画布深一档，靠一条 `line-dark` 上边线与正文分开，不用近黑块砸出一个视觉终点。

### Named Rules
**The One Meaning Rule.** 琥珀代表"可用 / 当前 / 关键"。装饰、边框、背景一律不得使用主色。一屏之内主色覆盖面积不超过 10%。

**The Zero-Chroma Neutral Rule.** 石墨族的中性色 **chroma 必须为 0**：底色不带任何主色色相。禁止 `#000` 与 `#fff`（用 `#121415` / `#fafbfb` 一类的近黑近白代替），也禁止把中性色染成暖调或冷调。审计方法：把任意一个灰放到 HSL 里，饱和度应为 0%；若某个灰看起来"发黄"或"发蓝"，它就不在系统里。这条正是 2026-09 改版的要点 —— 旧版把整个中性族染成了青调，与"底色应当安静、只有强调色说话"的目标相悖。
...
**混合制，但以色调分层为主。** 浅色模式下页面是一张连续画布，浮起的东西靠"更白的底 + 1px 发丝边 + 极轻的环境阴影"表达；深色模式下阴影全部失效，层级完全由**背景色阶**承担（画布 `#04121a` → 抬升面 `#0b2029` → 卡片面 `#0f2a33`），发丝线只做边界不做深度。禁止在深色模式用亮边框模拟浮起。
...
- **毛玻璃（Frosted）**：仅限固定与浮层元素：吸顶导航、底部 Tab Bar、右侧导航玻璃管、灯箱遮罩、主题下拉。全部保留 `backdrop-filter`，因为它们真的需要透视下方滚动内容。

### Shadow Vocabulary
- **ambient-hairline** (`box-shadow: 0 1px 2px rgb(15 87 79 / 0.06)`): 实心面板与轻色板的静置态。几乎不可见，只提供一丝离地感。
- **lift-on-hover** (`box-shadow: 0 4px 12px rgb(15 87 79 / 0.10)`): 卡片 hover 时的抬升。
- **floating** (`box-shadow: 0 6px 20px rgb(15 87 79 / 0.28)`): 固定元素的悬浮感。
- **overlay** (`box-shadow: 0 24px 70px rgb(4 18 26 / 0.5)`): 灯箱与下拉菜单。
...
- **Hover / Focus:** hover 加深到 `#0b463f`（仅颜色过渡，150ms）；键盘焦点用 4px `amber-400/60` 外环，永不隐藏。
- **Ghost / Secondary:** 白底 + 琥珀文字 + 琥珀浅边；hover 时底色转 `amber-50` 并把边框提到 `amber-400`。

### Chips
- **Style:** 胶囊形（`rounded-full`）。技术栈芯片为 `#eef0f1` 底 + `#4a6165` 文字，无边框；关键词芯片为 `amber-50` 底 + `amber-700` 文字 + `amber-200` 边。
