...
- **毛玻璃（Frosted）**：固定与浮层元素：**常驻顶部栏**、手机底部 Tab Bar、右侧导航玻璃管、灯箱遮罩、主题下拉。全部保留 `backdrop-filter`，因为它们真的需要透视下方滚动内容。
  ...
  - **文档区横栏**（`.docs-subbar`）走**同一套玻璃配方**（第九轮用户要求「把文档区的单独顶栏也和全局顶栏样式进行对齐」）：
    ...
    ① 多一层 72% 的 `background-color` —— 它是 sticky 的，正文会从它下面穿过；
       ⚠️ 第十一轮更新：这一层已改为**全透明**（`background-color: transparent`），
          否则正文"透过去"和折射都看不见。可读性由一条 `drop-shadow` 浅色晕保证（0 1px 3px / .85），
          断言同步改为「横栏的底必须是全透明」。
